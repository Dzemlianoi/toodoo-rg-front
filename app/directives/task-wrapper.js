...
app.directive('taskWrapper', function() {
  return {
    ...
    controller: ['$scope', 'Task', 'Flash', 'ngDialog',
      function ($scope, Task, Flash, ngDialog) {
        ...
        $scope.changePriority = function(task, response) {
          ...
          let sided_task = window._.find($scope.tasks, { id: response.sided_task.id });
          sided_task.priority = response.sided_task.priority
        };

        $scope.checkArrow = function(task){
          let max_task = window._.last($scope.tasks);
        ...
        $scope.setDeadline = function(newTime, oldTime, task) {
